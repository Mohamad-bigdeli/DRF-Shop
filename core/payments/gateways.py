...
class ZibalClient:
    def __init__(self, merchant_id: str = None, timeout: Union[int, float] = 10, 
                 sandbox: bool = False, enable_logging: bool = True):

        self.merchant_id = merchant_id or settings.ZIBAL_MERCHANT_ID
        self.sandbox = sandbox
        self.timeout = timeout
        self.enable_logging = enable_logging
        self.base_url = "https://gateway.zibal.ir/v1/" if not sandbox else "https://sandbox.zibal.ir/v1/"
        
        self._log(f"ZibalClient initialized with merchant_id: {self.merchant_id}, sandbox: {self.sandbox}")
    
    def payment_request(self, amount: int, callback_url: str, 
                       description: str = "Payment for order", mobile: str = None,
