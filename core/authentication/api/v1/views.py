...
class OtpVerifyView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not otp_cache:
                return Response({"detail":"The one-time code has expired or invalid phone. Request again."})
            if otp_cache == otp_code:
                user = User.objects.filter(phone=phone).first() if User.objects.filter(phone=phone).first() else None
                if user:
                    user.is_verified = True
                    user.save()
                    return Response({"detail":"The one-time code is correct and the user was verified."}, status=status.HTTP_200_OK)
                cache.delete(phone)
                return Response({"detail":"The one-time code is correct."}, status=status.HTTP_200_OK)
            return Response({"detail":"The one-time code is wrong!"}, status=status.HTTP_400_BAD_REQUEST)
        except Exception:
            return Response({"detail":"The operation was not performed"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)   
